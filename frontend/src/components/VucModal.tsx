...
export default function VucModal({ vuc, onClose, locale }: Props) {
  ...
  const perCapita = vuc.population > 0 ? Math.round(vuc.total_contracted_eur / vuc.population) : 0;
  const disclaimer = locale === 'sk'
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-[#0a0a0f] rounded-xl p-4 border border-[#1e1e2e]">
            ...
            <div className="text-[#94a3b8] text-sm mt-1">
              {locale === 'sk' ? 'Priame čerpanie' : 'Direct absorption'}
            </div>
          </div>
          <div className="bg-[#0a0a0f] rounded-xl p-4 border border-[#1e1e2e]">
        ...
        {perCapita > 0 && (
          <div className="mb-6 bg-[#0a0a0f] rounded-xl p-4 border border-[#1e1e2e]">
            ...
            <div className="text-[#94a3b8] text-sm mt-1">
              {locale === 'sk' ? 'Čerpanie na obyvateľa (priame)' : 'Per capita absorption (direct)'}
            </div>
          </div>
        )}
          ...
          <div className="mb-6 bg-[#0a0a0f] rounded-xl p-4 border border-[#1e1e2e] border-l-2 border-l-[#10b981]">
            ...
            {vuc.subsidiary_orgs.slice(0, 5).map((org, i) => (
              <div key={i} className="flex justify-between text-xs text-[#94a3b8] py-0.5">
                <span className="truncate flex-1 mr-2">{org.name}</span>
                <span className="font-mono text-[#10b981]/80 flex-shrink-0">{formatAmount(org.total_contracted_eur, locale)}</span>
              </div>
            ))}
          </div>
        )}
                ...
                <div key={i} className="bg-[#0a0a0f] rounded-lg p-3 border border-[#1e1e2e]">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-[#94a3b8]">
                      {p.stav.toLowerCase().includes('ukon') ? (locale === 'sk' ? 'Ukončený' : 'Completed') : (locale === 'sk' ? 'V realizácii' : 'In progress')}
                    </span>
                  </div>
                </div>
              ))}
