    ...
    <div className="fixed inset-0 z-50 flex justify-center items-start overflow-y-auto bg-black/60 p-4 pt-[10vh]" onClick={onClose}>
      <div className="bg-[#13131a] border border-[#1e1e2e] rounded-2xl w-full max-w-[720px] p-6 sm:p-8 animate-fade-in-up overflow-y-auto" style={{ maxHeight: '80vh', WebkitOverflowScrolling: 'touch' } as React.CSSProperties} onClick={e => e.stopPropagation()}>
        ...
        {detailLoading && !m ? (
          ...
          {topProjects.length > 0 && <div className="mb-6"><h3 className="text-sm font-medium text-[#94a3b8] mb-3 uppercase tracking-wider">{tr.modal_top_projects}</h3><div className="space-y-2">{topProjects.map((p, i) => { const act = !p.stav.toLowerCase().includes('ukončen'); const ed = p.datumKoncaRealizacie ? (() => { const d = new Date(p.datumKoncaRealizacie); return isNaN(d.getTime()) ? null : `${d.getMonth()+1}/${d.getFullYear()}`; })() : null; return <div key={i} className={`bg-[#0a0a0f] rounded-lg p-3 border border-[#1e1e2e] ${p.isMikroregion ? 'border-l-2 border-l-[#8b5cf6]/40' : ''}`}><div className="text-sm text-[#f8fafc] mb-1 line-clamp-2">{p.nazov}{p.isMikroregion && <span className="ml-1.5 text-[10px] text-[#8b5cf6] bg-[#8b5cf6]/10 rounded px-1 py-0.5 font-medium align-middle">mikroregión</span>}</div>{p.isMikroregion && p.fullProjectSum ? <div className="text-[10px] text-[#94a3b8]/40 mb-1">{locale === 'sk' ? 'z celkových' : 'of total'} {formatAmount(p.fullProjectSum, locale)} · {p.source}</div> : null}{act && ed && <div className="text-xs text-[#94a3b8]/60 mb-1">{locale === 'sk' ? 'Realizácia do' : 'Until'}: {ed}</div>}<div className="flex justify-between text-xs"><span className={`font-mono ${p.isMikroregion ? 'text-[#8b5cf6]' : 'text-[#3b82f6]'}`}>{formatAmount(p.sumaZazmluvnena, locale)}</span><span className="text-[#94a3b8]">{act ? tr.active : tr.completed}</span></div></div>; })}</div></div>}
